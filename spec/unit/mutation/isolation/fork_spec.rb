...
RSpec.describe Mutation::Isolation::Fork do
  ...
  describe '#call' do
    ...
    context 'when no IO operation fails' do
      ...
      specify do
        XSpec::ExpectationVerifier.verify(self, expectations) do
          expect(subject).to eql(Mutation::Isolation::Result::Success.new(block_return))
        end
      end
    end
    ...
    context 'when expected exception was raised when reading from child' do
      [ArgumentError, EOFError].each do |exception_class|
        context "on #{exception_class}" do
          ...
          specify do
            XSpec::ExpectationVerifier.verify(self, expectations) do
              expect(subject).to eql(Mutation::Isolation::Result::Exception.new(exception))
            end
          end
        end
      end
    ...
    context 'when child exits nonzero' do
      ...
      let(:expected_result) do
        Mutation::Isolation::Result::ErrorChain.new(
          described_class::ChildError.new(status_error),
          Mutation::Isolation::Result::Success.new(block_return)
        )
      end
