...
module RegexpSpec
  ...
  RSpec.shared_context 'regexp transformation' do
    let(:parsed)     { Mutation::AST::Regexp.parse(regexp)      }
    let(:ast)        { Mutation::AST::Regexp.to_ast(parsed)     }
    let(:expression) { Mutation::AST::Regexp.to_expression(ast) }
  ...
  def self.expect_mapping(regexp, type, &block)
    RSpec.describe Mutation::AST::Regexp::Transformer.lookup(type) do
