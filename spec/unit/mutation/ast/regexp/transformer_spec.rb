...
RSpec.describe Mutation::AST::Regexp::Transformer do
  before do
    stub_const("#{described_class}::REGISTRY", Mutation::Registry.new)
  end
  ...
  it 'rejects duplicate registrations' do
    ...
    expect { Class.new(described_class) { register(:regexp_bos_anchor) } }
      .to raise_error(Mutation::Registry::RegistryError)
