...
RSpec.describe Mutation::Reporter::CLI::Printer do
  ...
  let(:reportable) { instance_double(Mutation::Result::Env, success?: success?) }
  let(:tty?)       { true                                                     }
  let(:success?)   { true                                                     }
  ...
  describe '#status' do
    ...
    context 'on tty' do
      context 'on success' do
        it_reports Mutation::Color::GREEN.format('foo bar') + "\n"
      end

      context 'on failure' do
        let(:success?) { false }
        it_reports Mutation::Color::RED.format('foo bar') + "\n"
      end
    end
  ...
  describe '#colorize' do
    let(:class_under_test) do
      Class.new(described_class) do
        def run
          puts(colorize(Mutation::Color::RED, 'foo'))
        end
      end
    end

    context 'when output is a tty?' do
      it_reports Mutation::Color::RED.format('foo') + "\n"
    end
