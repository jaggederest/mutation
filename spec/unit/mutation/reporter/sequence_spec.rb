...
RSpec.describe Mutation::Reporter::Sequence do
  let(:object)     { described_class.new([reporter_a, reporter_b]) }
  let(:value)      { instance_double(Object)                       }
  let(:reporter_a) { instance_double(Mutation::Reporter, delay: 1.0) }
  let(:reporter_b) { instance_double(Mutation::Reporter, delay: 2.0) }
