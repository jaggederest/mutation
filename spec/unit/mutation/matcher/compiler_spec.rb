...
RSpec.describe Mutation::Matcher::Compiler, '#call' do
  ...
  context 'on empty config' do
    ...
    let(:expected_positive_matcher) { Mutation::Matcher::Chain.new([]) }
  ...
  context 'on config with match expression' do
    ...
    let(:expected_positive_matcher)    { Mutation::Matcher::Chain.new([matcher_a]) }
    ...
    context 'and ignore expressions' do
      ...
      let(:ignore_expression_predicates) do
        [Mutation::Matcher::Compiler::SubjectPrefix.new(expression_b)]
      end
    ...
    context 'and subject filters' do
      let(:subject_filter) { instance_double(Mutation::Matcher) }
