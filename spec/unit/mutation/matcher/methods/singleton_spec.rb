...
RSpec.describe Mutation::Matcher::Methods::Singleton, '#call' do
  ...
  let(:subject_a) { instance_double(Mutation::Subject, 'A') }
  let(:subject_b) { instance_double(Mutation::Subject, 'B') }
  let(:subject_c) { instance_double(Mutation::Subject, 'C') }

  let(:subjects) { [subject_a, subject_b, subject_c] }

  before do
    matcher = Mutation::Matcher::Method::Singleton

    {
      ...
      allow(matcher).to receive(:new)
        ...
        .and_return(Mutation::Matcher::Static.new([subject]))
    end
  end
