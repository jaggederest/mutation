...
RSpec.describe Mutation::Env::Bootstrap do
  let(:matcher_config)       { Mutation::Matcher::Config::DEFAULT     }
  ...
  let(:config) do
    Mutation::Config::DEFAULT.with(
      ...
      reporter:    instance_double(Mutation::Reporter),
      requires:    []
    )
  end
  ...
  let(:expected_env) do
    Mutation::Env.new(
      ...
      selector:         Mutation::Selector::Expression.new(integration),
      subjects:         []
    )
  end
  ...
  describe '.call' do
    ...
    context 'when Module#name calls result in exceptions' do
      ...
      let(:expected_warning) do
        ...
        "RuntimeError. #{Mutation::Env::SEMANTICS_MESSAGE}"
      end
    ...
    context 'when Module#name does not return a String or nil' do
      ...
      let(:expected_warning) do
        ...
        "returned Object. #{Mutation::Env::SEMANTICS_MESSAGE}"
      end
    ...
    context 'when scope matches expression' do
      ...
      let(:expected_env) do
        subjects = Mutation::Matcher::Scope.new(TestApp::Literal).call(Fixtures::TEST_ENV)

        super().with(
          matchable_scopes: [
            Mutation::Scope.new(TestApp::Empty,   match_expressions.last),
            Mutation::Scope.new(TestApp::Literal, match_expressions.first)
          ],
