...
RSpec.describe Mutation::Zombifier do
  ...
  let(:ruby_vm) do
    MutationSpec::RubyVM.new(
      [
        MutationSpec::RubyVM::EventExpectation::Require.new(
          expected_payload: {
            logical_name: 'project'
          },
          return_value:     true
        ),
        MutationSpec::RubyVM::EventExpectation::Eval.new(
          expected_payload: {
  ...
  let(:file_system) do
    MutationSpec::FileSystem.new(
      Hash[
        file_entries.map { |key, attributes| [key, MutationSpec::FileState.new(attributes)] }
      ]
    )
  end
