...
RSpec.describe Mutation::Selector::Expression do
  describe '#call' do
    ...
    let(:context)          { instance_double(Mutation::Context)                                        }
    let(:node)             { instance_double(Parser::AST::Node)                                      }
    let(:integration)      { instance_double(Mutation::Integration, all_tests: all_tests)              }
    let(:test_a)           { instance_double(Mutation::Test, expression: parse_expression('SubjectA')) }
    let(:test_b)           { instance_double(Mutation::Test, expression: parse_expression('SubjectB')) }
    let(:test_c)           { instance_double(Mutation::Test, expression: parse_expression('SubjectC')) }
