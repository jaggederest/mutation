      ...
      private
        ...
        def emit_argument_presence
          ...
          Util::Array::Presence.call(children).each do |children|
            if children.one? && n_mlhs?(Mutation::Util.one(children))
              emit_procarg(Mutation::Util.one(children))
            else
              emit_type(*children)
            end
          end
        ...
        def emit_argument_mutations
          children.each_with_index do |child, index|
            Mutator.mutate(child).each do |mutation|
              next if invalid_argument_replacement?(mutation, index)
        ...
        #
        # @return [Boolean]
        def invalid_argument_replacement?(mutation, index)
          n_arg?(mutation) && children[0...index].any?(&method(:n_optarg?))
        end
