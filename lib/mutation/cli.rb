  ...
  private
    ...
    def parse(arguments)
      opts = OptionParser.new do |builder|
        builder.banner = 'usage: mutation [options] MATCH_EXPRESSION ...'
    ...
    def add_environment_options(opts)
      ...
      opts.on('--zombie', 'Run mutation zombified') do
        with(zombie: true)
      end
    ...
    def setup_integration(name)
      ...
      raise Error, "Could not load integration #{name.inspect} (you may want to try installing the gem mutation-#{name})"
    end
    ...
    def add_debug_options(opts)
      ...
      opts.on('--version', 'Print mutations version') do
        puts("mutation-#{VERSION}")
        config.kernel.exit
      end
