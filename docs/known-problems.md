...
Occasionally mutation will produce a mutation with an infinite runtime. When this happens
mutation will look like it is running indefinitely without killing a remaining mutation. To
...
Mutations generated by mutation can cause MRI to enter VM states its not prepared for.
...
* MRI crashes with a segfault. Mutation kills each mutation in a dedicated fork to isolate
  the mutations side effects when this fork terminates abnormally (segfault) Mutation
  counts the mutation as killed.

* MRI crashes with a segfault and gets stuck when handling the segfault.
  Depending on the number of active kill jobs mutation might appear to continue normally until
  all workers are stuck into this state when it begins to hang.
  Currently mutation must assume that your test suite simply not terminated yet as from the outside
...
* [Mutation issue](https://github.com/mbj/mutation/issues/265)
